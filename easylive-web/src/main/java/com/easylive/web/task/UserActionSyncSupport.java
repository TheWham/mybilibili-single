...
public class UserActionSyncSupport {
    ...
    private void flushUserCoinCount(Map<String, Integer> countMap) {
        List<UserCoinCountUpdateDTO> updateList = new ArrayList<>();
        for (Map.Entry<String, Integer> entry : countMap.entrySet()) {
            if (entry.getValue() == 0) {
                continue;
            }
            updateList.add(new UserCoinCountUpdateDTO(entry.getKey(), entry.getValue() < 0 ? 0 : entry.getValue(), entry.getValue()));
        }
        if (!updateList.isEmpty()) {
